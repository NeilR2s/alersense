...
const uint8_t BUZZER_OFF_LEVEL = HIGH;
...
void updateBuzzer()
{
    ...
    if (buzzer.isOn)
    {
        ...
        {
            digitalWrite(PIN_BUZZ, BUZZER_OFF_LEVEL);
            buzzer.isOn = false;
    ...
    else
    {
        ...
        {
            digitalWrite(PIN_BUZZ, BUZZER_ON_LEVEL);
            buzzer.isOn = true;
            buzzer.lastTransition = now;
        }
    }
...
void setup()
{
    ...
    // This buzzer module is wired active-low: LOW sounds, HIGH silences.
    digitalWrite(PIN_BUZZ, BUZZER_OFF_LEVEL);
